...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; }
    ...
    .download-btn { width:150px; height:38px; justify-self:end; background:#F5F2E8; color:#141414; border:0; border-radius:7px; padding:9px 12px; font:600 14px/1 system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif; cursor:pointer; }
...
<body>
<main>
  <div class="topbar"><select id="date" aria-label="Snapshot date"></select><h1 class="dashboard-title">Liquid Leadership</h1><button id="download-image" class="download-btn" type="button">Download</button></div>
...
if (!history.length) { document.querySelector('main').innerHTML = '<p class="empty">Run the scanner once to create a momentum-leader snapshot.</p>'; } else { updateDates(); dateSelect.addEventListener('change', render); downloadButton.addEventListener('click', downloadPageImage); window.addEventListener('resize', render); render(); }
</script>
</body>
</html>'''
